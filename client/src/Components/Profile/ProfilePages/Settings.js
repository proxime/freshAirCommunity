...
const Settings = ({
  ...
  const onSubmit = (e) => {
    ...
    if (changePassword) {
      if (!password || !password2)
        return setAlert({
          msg: "Wypełnij pola formularza!",
          param: "password",
        });
      if (password === password2)
        return setAlert({
          msg: "Wprowadziłeś takie same hasła!",
          param: "password",
        });
  ...
  const successPasswordAlert = alert.filter(
    (item) => item.param === "successPassword"
  );
  ...
  return (
    <>
      ...
      <div className="edit-profile">
        <div className="edit-profile-image">
          <div
            ...
            style={{ backgroundImage: `url(${usedAvatar})` }}
          >
            <div
        ...
        <div className="edit-profile-data">
          <form onSubmit={(e) => onSubmit(e)}>
            ...
            <p onClick={() => setChangePassword(!changePassword)}>
            ...
            {changePassword && (
              <div className="edit-profille-password">
                ...
                {passwordAlert.length > 0 && (
                  <span>{passwordAlert[0].msg}</span>
                )}
                {successPasswordAlert.length > 0 && (
                  <span style={{ color: "green" }}>
                    {successPasswordAlert[0].msg}
                  </span>
                )}
              </div>
            )}
          ...
          <div className="setting-buttons">
            <button className="settings-save" onClick={(e) => onSubmit(e)}>
              Zapisz Zmiany
            </button>
      ...
      {openIconWindow && (
        <SettingsIcons
          setIconsWindowH={(height) => setIconsWindowH(height)}
          setOpenIconWindow={(value) => setOpenIconWindow(value)}
        />
      )}
    </>
  );
