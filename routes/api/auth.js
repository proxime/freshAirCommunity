...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (userLogin) {
        errorsTable.push({
          msg: "Podany Login jest już w użyciu",
          param: "login",
        });
      }
      ...
      if (userMail) {
        errorsTable.push({
          msg: "Podany E-mail jest już w użyciu",
          param: "email",
        });
      }
    ...
    try {
      ...
      if (!user) {
        return res.status(400).json({
          errors: [{ msg: "Wprowadziłeś nieprawidłowe dane", param: "data" }],
        });
      }
...
router.post(
  "/change/email",
  [auth, [check("email", "Podaj prawidłowy adres E-mail").isEmail()]],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    ...
    try {
      ...
      if (emailExist) {
        ...
        return res.status(400).json({
          errors: [{ msg: "Podany E-mail jest już zajęty", param: "email" }],
        });
      }
...
router.post(
  ...
  [
    ...
    [
      check("password", "Wprowadź hasło").exists(),
      check("password2", "Hasło musi posiadać 6 do 16 znaków").isLength({
        min: 6,
        max: 16,
      }),
    ],
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!isMath) {
        return res.status(400).json({
          errors: [
            { msg: "Wprowadziłeś błędnie aktualne hasło", param: "password" },
          ],
        });
      }
      ...
      res.json({
        alerts: [{ msg: "Hasło zostało zmienione", param: "successPassword" }],
      });
    } catch (err) {
      console.error(err.message);
      res.status(500).send("Server Error");
...
router.post("/change/avatar", auth, async (req, res) => {
  ...
  try {
    if (isNaN(Number(avatar)) || Number(avatar) < 0 || Number(avatar) > 14)
      return res.status(400).json({
        errors: [{ msg: "Wybierz prawidłowy avatar", param: "avatar" }],
      });
    ...
    res.json({
      alerts: [{ msg: "Avatar został pomyślnie zmieniony", param: "avatar" }],
    });
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server Error");
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (isUsed.length > 0) {
        ...
        return res.json({
          alerts: [{ msg: "Już nie lubisz tego miasta", param: "cityLikes" }],
        });
      }
      ...
      res.json({
        alerts: [{ msg: "Polubiono nowe miasto", param: "cityLikes" }],
      });
    } catch (err) {
      console.error(err.message);
      res.status(500).send("Server Error");
